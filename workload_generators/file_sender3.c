...
int main(int argc, char *argv[])
{
	...
	FILE *f;
	char filaname[FNAME_SIZ];
	int data_offset;
	int data_load;
	int packet_id;

	strcpy(filaname, argv[5]);
	packet_id = atoi(argv[6]);
	data_offset = atoi(argv[7]);
	data_load = atoi(argv[8]);
	...
	printf("identification,PCP,ID: %d %d %d",identification,PCP,ID);
